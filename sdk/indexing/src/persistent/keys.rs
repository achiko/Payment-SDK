...
use crate::{BlockHeight, EventCursor, IndexScope, ObservationRevision, RebuildGeneration};

pub(super) const NAMESPACE: &str = "ix.semantic.v1";
...
fn transaction_component(key: &mut Vec<u8>, chain: &str, value: &str) {
    component(key, chain.as_bytes());
...
#[must_use]
pub(super) fn policy_migration_counter(scope: &IndexScope) -> Key {
    Key(scope_prefix(scope, POLICY_MIGRATION_COUNTER))
}

#[must_use]
pub(super) fn policy_migration(scope: &IndexScope, version: u64) -> Key {
    let mut key = scope_prefix(scope, POLICY_MIGRATION);
    key.extend_from_slice(&version.to_be_bytes());
    Key(key)
}

#[must_use]
pub(super) fn policy_migration_id(scope: &IndexScope, idempotency_key: &str) -> Key {
...
pub(super) fn current_observation(
    ...
    transaction_component(&mut key, chain, transaction_id);
    Key(key)
}
...
pub(super) fn address_transaction(
    ...
    address_chain: &str,
    address: &str,
    transaction_chain: &str,
    transaction_id: &str,
) -> Key {
    let mut key = address_transaction_prefix(scope, generation, address_chain, address);
    transaction_component(&mut key, transaction_chain, transaction_id);
    Key(key)
}
...
pub(super) fn address_transaction_prefix(
    ...
    component(&mut key, address_chain.as_bytes());
    component(&mut key, address.as_bytes());
    key
}
