...
use json_rpc::JsonRpcClient;
use serde_json::{Map, Value};

use crate::{BitcoinNetwork, BitcoinTransactionId};

use super::{
    BitcoinBlock, BitcoinIndexRpc, BitcoinIndexingCapabilities, BitcoinOutPoint,
    model::{address_for_script, parse_block},
};
use crate::rpc::{
    BitcoinCoreClient, BitcoinCoreConfig, format_bitcoin_block_hash, parse_bitcoin_block_hash,
...
pub struct BitcoinIndexSourceConfig {
    pub scope: IndexScope,
    pub network: BitcoinNetwork,
    pub expected_genesis_hash: BlockHash,
}

impl BitcoinIndexSourceConfig {
    pub fn validate(&self) -> Result<(), SourceError> {
        if self.scope.chain != ChainId("bitcoin".to_owned()) {
...
pub struct BitcoinCoreBlockSource<C> {
    core: BitcoinCoreClient<C>,
    config: BitcoinIndexSourceConfig,
}

impl<C> BitcoinCoreBlockSource<C>
where
    C: JsonRpcClient,
{
    pub async fn connect(client: C, config: BitcoinIndexSourceConfig) -> Result<Self, SourceError> {
        config.validate()?;
        let core = BitcoinCoreClient::connect(
            client,
            BitcoinCoreConfig {
    ...
    #[must_use]
    pub fn config(&self) -> &BitcoinIndexSourceConfig {
        &self.config
    }

    #[must_use]
    pub fn core(&self) -> &BitcoinCoreClient<C> {
        &self.core
    }
    ...
    async fn resolve_external_transaction(
        &self,
        transaction_id: BitcoinTransactionId,
        required_outputs: &BTreeSet<u32>,
    ) -> Result<ExternalTransactionEvidence, SourceError> {
        let raw = self
            .core
            .request_optional_result(
                "getrawtransaction",
        ...
        Ok(ExternalTransactionEvidence { outputs })
    }
    ...
    async fn enrich_prevouts(&self, raw_block: Vec<u8>) -> Result<Vec<u8>, SourceError> {
        ...
        let mut external_outputs: BTreeMap<BitcoinTransactionId, BTreeSet<u32>> = BTreeMap::new();
        ...
        for (transaction_id, transaction) in &transactions {
            for input in &transaction.input {
                ...
                let previous_id = BitcoinTransactionId::from(input.previous_output.txid);
            ...
            for output_index in 0..transaction.output.len() {
                ...
                earlier_outputs.insert(BitcoinOutPoint {
                    transaction_id: *transaction_id,
                    output_index,
                });
            }
        }
        let compact_evidence_budget = external_prevout_count
            .checked_mul(MAX_COMPACT_PREVOUT_JSON_BYTES)
            .ok_or_else(|| {
                source_error("Bitcoin compact prevout evidence budget overflowed", false)
            })?;
        if compact_evidence_budget > MAX_COMPACT_PREVOUT_TOTAL_BYTES {
            return Err(source_error(
                "Bitcoin compact prevout evidence exceeds its aggregate bound",
                false,
            ));
        }
        ...
        let requests =
            external_outputs
                ...
                .map(|(transaction_id, output_indexes)| async move {
                    self.resolve_external_transaction(transaction_id, &output_indexes)
                        .await
                        .map(|evidence| (transaction_id, evidence))
                });
        let mut requests = stream::iter(requests).buffer_unordered(MAX_IN_FLIGHT_PREVOUT_REQUESTS);
        while let Some(result) = requests.next().await {
            let (transaction_id, evidence) = result?;
            resolved.insert(transaction_id, evidence);
        }
            ...
            transactions.iter().zip(transaction_values)
        {
            ...
            for (input, native_input) in inputs.iter_mut().zip(&transaction.input) {
                ...
                let previous_id = BitcoinTransactionId::from(native_input.previous_output.txid);
                let outpoint = BitcoinOutPoint {
                    transaction_id: previous_id,
                ...
                let prevout = previous_output_json(output)?;
    ...
    ) -> Result<Option<BitcoinBlock>, SourceError> {
        let Some(raw_block) = self.raw_block(hash).await? else {
            return Ok(None);
        };
        let raw_block = self.enrich_prevouts(raw_block).await?;
        let parsed = parse_block(&raw_block, expected_height, Some(hash), self.config.network)
            .map_err(|error| source_error(error.to_string(), true))?;
        Ok(Some(BitcoinBlock {
...
#[derive(Clone, Debug, PartialEq, Eq)]
struct ExternalOutputEvidence {
    value_satoshis: u64,
    address: Option<crate::BitcoinAddress>,
}

#[derive(Clone, Debug, PartialEq, Eq)]
struct ExternalTransactionEvidence {
    outputs: BTreeMap<u32, ExternalOutputEvidence>,
}

fn decode_consensus_transaction(
    ...
    if BitcoinTransactionId::from(transaction.compute_txid()) != expected_id {
        return Err(source_error(
...
fn validate_input_claims(value: &Value, transaction: &Transaction) -> Result<(), SourceError> {
    let inputs = value
        .as_object()
...
fn record_external_prevout(
    outputs: &mut BTreeMap<BitcoinTransactionId, BTreeSet<u32>>,
    count: &mut usize,
    transaction_id: BitcoinTransactionId,
    output_index: u32,
) -> Result<(), SourceError> {
...
fn validate_compact_address(address: Option<&crate::BitcoinAddress>) -> Result<(), SourceError> {
    let Some(address) = address else {
        return Ok(());
    };
    if address.0.len() > MAX_COMPACT_ADDRESS_BYTES
        || !address.0.bytes().all(|byte| byte.is_ascii_alphanumeric())
    {
        return Err(source_error(
            "Bitcoin canonical prevout address exceeds the compact evidence bound",
            false,
        ));
    }
    Ok(())
...
fn previous_output_json(output: &ExternalOutputEvidence) -> Result<Value, SourceError> {
    validate_compact_address(output.address.as_ref())?;
    let prevout = serde_json::json!({
        "value_satoshis": output.value_satoshis,
        "address": output.address.as_ref().map(|address| address.0.as_str()),
    });
...
impl<C> BlockSource for BitcoinCoreBlockSource<C>
where
    C: JsonRpcClient,
{
...
impl<C> BitcoinIndexRpc for BitcoinCoreBlockSource<C>
where
    C: JsonRpcClient,
{
    fn indexing_capabilities(&self) -> BitcoinIndexingCapabilities {
        BitcoinIndexingCapabilities::V1
    }
    ...
    fn block_by_hash<'a>(
        ...
        Box::pin(async move { self.fetch_block(&hash, None).await })
    }
}
...
mod tests {
    ...
    use json_rpc::{JsonRpcError, JsonRpcFailure, JsonRpcRequest, JsonRpcResponse, RawJson};
    ...
    fn config() -> BitcoinIndexSourceConfig {
    ...
    fn numbered_block_fetch_retains_enriched_result_and_rechecks_canonical_hash() {
        ...
        let source = block_on(BitcoinCoreBlockSource::connect(
        ...
        assert_eq!(
            serde_json::from_slice::<Value>(&block.raw_block)
    ...
    fn external_prevouts_are_resolved_once_from_narrow_bounded_calls() {
        ...
        let source = block_on(BitcoinCoreBlockSource::connect(client, config()))
            .expect("valid scripted source must connect");
        ...
        let raw: Value =
            serde_json::from_slice(&block.raw_block).expect("enriched block JSON must decode");
        ...
        for input in inputs {
            let prevout = input
                .get("prevout")
                .expect("external input must retain compact evidence");
            ...
            assert!(
                serde_json::to_vec(prevout)
                    .expect("compact evidence must encode")
                    .len()
                    <= MAX_COMPACT_PREVOUT_JSON_BYTES
            );
        }
        assert!(
            block.raw_block.len() < previous.output[2].script_pubkey.len(),
    ...
    #[test]
    fn external_prevout_lookup_must_return_confirmed_transaction_evidence() {
        ...
        let error = block_on(source.block_at(BlockHeight(10)))
            .expect_err("mempool-only previous-output evidence must retry");
        calls.assert_exhausted();
    ...
    fn external_prevout_bound_is_above_consensus_maximum_and_fails_before_growth() {
        ...
        let transaction_id = BitcoinTransactionId([0x33; 32]);
        let mut outputs = BTreeMap::new();
        let mut count = 0;
    ...
    fn disappearing_height_is_optional_for_canonical_hash_and_retryable_for_tip() {
        ...
        let canonical = block_on(BitcoinCoreBlockSource::connect(
        ...
        let tip = block_on(BitcoinCoreBlockSource::connect(
            ScriptedClient::new(tip_replies),
            config(),
        ))
        .expect("valid scripted source must connect");
