...
use bitcoin::{
    BlockHash as NativeBlockHash, ScriptBuf, Transaction, Txid, consensus, hashes::Hash,
    hex::FromHex,
};
use indexing::{BlockHash, BlockHeight, BlockRef};
use serde_json::{Map, Number, Value};

use crate::{BitcoinNetwork, BitcoinTransactionId, Satoshi};

use super::BitcoinOutPoint;
...
pub(super) struct ParsedInput {
    pub previous_output: Option<ParsedPreviousOutput>,
}

#[derive(Clone, Debug, PartialEq, Eq)]
pub(super) struct ParsedPreviousOutput {
    pub outpoint: BitcoinOutPoint,
    pub value: Satoshi,
    pub address: Option<crate::BitcoinAddress>,
}
...
pub(super) fn parse_block(
    ...
    for transaction in transaction_values {
        let transaction = parse_transaction(transaction, height, network, &same_block_outputs)?;
...
fn parse_transaction(
    ...
    network: BitcoinNetwork,
    same_block_outputs: &BTreeMap<BitcoinOutPoint, ParsedPreviousOutput>,
    ...
    let native: Transaction = consensus::deserialize(&raw)
        .map_err(|_| ParseError::new("Bitcoin transaction consensus bytes are invalid"))?;
    if BitcoinTransactionId::from(native.compute_txid()) != txid {
        return Err(ParseError::new(
    ...
    for (index, (input, native_input)) in input_values.iter().zip(&native.input).enumerate() {
        ...
        let previous_output = match object.get("prevout").and_then(Value::as_object) {
            Some(prevout) => {
                let resolved = parse_previous_output(prevout, outpoint, block_height, network)?;
    ...
    for (position, (output, native_output)) in output_values.iter().zip(&native.output).enumerate()
    {
...
fn parse_previous_output(
    prevout: &Map<String, Value>,
    outpoint: BitcoinOutPoint,
    spending_height: BlockHeight,
    network: BitcoinNetwork,
) -> Result<ParsedPreviousOutput, ParseError> {
    if let Some(value) = prevout.get("value_satoshis") {
            ...
            .ok_or_else(|| ParseError::new("Bitcoin compact prevout address fact is missing"))?
        {
            ...
            Value::String(address) => {
                let canonical = crate::BitcoinAddress::parse_for_network(address, network)
                    .map_err(|_| {
                        ParseError::new(
                            "Bitcoin compact prevout address is invalid or wrong-network",
                        )
                    })?;
                if canonical.0 != *address {
                    return Err(ParseError::new(
        ...
        return Ok(ParsedPreviousOutput {
            outpoint,
            value: Satoshi(value),
            address,
        });
...
pub(super) fn address_for_script(
    script: &ScriptBuf,
    network: BitcoinNetwork,
) -> Option<crate::BitcoinAddress> {
    bitcoin::Address::from_script(script, network.native())
        .ok()
        .map(|address| crate::BitcoinAddress(address.to_string()))
}
...
fn parse_txid(value: &str) -> Result<BitcoinTransactionId, ParseError> {
    value
