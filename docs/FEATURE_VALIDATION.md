...
| Solana primitives and witnessed account acquisition | `sdk/chains/solana`, `sdk/wallets`, `packages/json-rpc`, `apps/api` error projection | Canonical 32-byte Base58 addresses retain off-curve read/index support while `NativeDestination` enforces the send-only curve gate. `Lamport` performs exact nine-decimal checked conversion through `u64::MAX`; native identity remains `{ solana, native }`. Exact lowercase-hex Ed25519 seeds enter shared zeroizing `SecretBytes`; OS generation has a private failure seam; maintained-library fixtures prove exact address, message signature, local verification, and first-signature ID. One redacted endpoint-affine `request_once` client implements health, genesis, confirmed/finalized slots, strict full Base64 account reads, one 1..=100 multi-account call, and finalized balance with typed transport/HTTP/remote/malformed/resource/floor failures. Canonical process-local source leases, truthful `SourceBusy` 503 projection, stable source/destination deduplication, `F -> C -> U` witnessing, System-account classification, atomic floor/balance handoff, and cancellation at every RPC await pass with owned doubles. Fifty-seven Solana tests, one doctest, JSON-RPC loopback tests, wallet/API error tests, strict Clippy, formatting, and design-lint pass; no Solana provider/sender, submission, indexer, or runtime composition is claimed. |
...
| Canonical plain Base58 and native SOL wallet values | Canonical Base58 ADR and Solana requirements | address, curve, checked-lamport, native-identity, Ed25519 key ownership/signing, and exact finalized balance primitives exist; public API Solana/Base58 variants and the concrete `wallets::Provider`/`Wallet` adapter remain unimplemented |
| Native SOL block interpretation and canonical history | Native SOL History and Indexing & Central Database ADRs | the Solana package has a validated scope-bound interpreter owner, but it does not implement `indexing::BlockInterpreter`; no tests for legacy/version-0 loaded addresses, top-level/inner System transfers, exact fee/status, failed-transaction movement suppression, completeness shielding, or empty UTXO changes exist |
...
| Native SOL construction, submission, exact-byte replay, and ambiguity reconciliation | Native SOL Submission ADR | process-local lexical source coordination, Ed25519 signing, recent-blockhash lifetime, and fixed-width opaque Memo token values exist, but no transaction/message/Memo-v3 instruction builder, executable-Memo readiness probe, fee/simulation RPC, guarded envelope transition, exact-signature broadcaster, three-call replay bound, checkpoint-stable indexed absence proof, background reconciliation, or duplicate-risk tests exist |
...
| Sparse finalized Solana traversal and retained-reorg evidence | Indexing & Central Database ADR | generic sparse-position synchronization and retained-reorg fixtures now pass, and the Solana package owns a bounded produced-block budget, but no Solana block source or native sparse-slot adapter exists; runtime composition is also unimplemented |
