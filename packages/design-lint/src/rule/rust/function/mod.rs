...
impl Rule for FreeFunction {
    ...
    fn check(&self, workspace: &Workspace) -> Result<Vec<Finding>> {
        ...
        let mut references =
            HashMap::<String, Vec<crate::rule::support::references::Reference>>::new();
...
impl Candidate {
    fn finding<'a>(
        ...
        finding.help =
            "make it a method on that type or add a temporary #[design_review::classify(...)] classification".into();
        ...
        let review = Review {
            ...
            questions: vec![
                "Does this express the argument's own behavior, or a transformation over it?"
                    .into(),
                ...
                .is_ok_and(|derives| {
                    derives.iter().any(|derive| {
                        derive.segments.last().is_some_and(|segment| {
                            matches!(segment.ident.to_string().as_str(), "Parser" | "Args")
                        })
                    })
                })
    })
