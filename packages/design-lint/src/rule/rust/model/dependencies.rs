...
pub(super) fn local_dependencies(path: &Path) -> Result<HashSet<(String, String)>> {
    ...
    let text = fs::read_to_string(&manifest).map_err(|error| {
        LintError::configuration(format!("read {}: {error}", manifest.display()))
    })?;
    let value = toml::from_str::<toml::Value>(&text).map_err(|error| {
        LintError::configuration(format!("decode {}: {error}", manifest.display()))
    })?;
    ...
    let workspace_dependencies = workspace
        ...
        .map(|path| {
            let text = fs::read_to_string(path).map_err(|error| {
                LintError::configuration(format!("read {}: {error}", path.display()))
            })?;
            let value = toml::from_str::<toml::Value>(&text).map_err(|error| {
                LintError::configuration(format!("decode {}: {error}", path.display()))
            })?;
            Ok::<_, LintError>(
                value
                    .get("workspace")
    ...
    for (alias, specification) in dependencies {
        ...
        if let Some(dependency) = local_package(specification, &manifest)?
...
fn package_name(manifest: &Path) -> Result<String> {
    ...
    value
        ...
        .map(ToOwned::to_owned)
        .ok_or_else(|| {
            LintError::configuration(format!("{} has no package.name", manifest.display()))
        })
}
...
mod tests {
    ...
    fn accepts_valid_manifest_without_dependencies() {
        let root =
            std::env::temp_dir().join(format!("lint-model-valid-manifest-{}", std::process::id()));
        let _ = fs::remove_dir_all(&root);
        fs::create_dir_all(root.join("src")).unwrap();
        fs::write(root.join("Cargo.toml"), "[package]\nname = \"sample\"\n").unwrap();
        assert!(
            local_dependencies(&root.join("src/lib.rs"))
                .unwrap()
                .is_empty()
        );
        fs::remove_dir_all(root).unwrap();
    ...
    fn rejects_unresolved_workspace_dependency() {
        let root = std::env::temp_dir().join(format!(
            "lint-model-workspace-dependency-{}",
            std::process::id()
        ));
        let _ = fs::remove_dir_all(&root);
    ...
    fn rejects_non_string_local_dependency_path() {
        let root =
            std::env::temp_dir().join(format!("lint-model-local-path-type-{}", std::process::id()));
        let _ = fs::remove_dir_all(&root);
        fs::create_dir_all(root.join("member/src")).unwrap();
        fs::write(
            root.join("member/Cargo.toml"),
