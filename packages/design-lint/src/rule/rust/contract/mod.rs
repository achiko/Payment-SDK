...
fn separation_evidence(clusters: &BTreeMap<&'static str, Vec<&Method>>) -> usize {
    ...
    for (left, right) in profile_pairs(&profiles)
        .filter(|(left, right)| distinct_profile(&profiles[*left], &profiles[*right]))
    {
...
fn profile_pairs(profiles: &[Profile]) -> impl Iterator<Item = (usize, usize)> + '_ {
    (0..profiles.len())
        .flat_map(move |left| (left + 1..profiles.len()).map(move |right| (left, right)))
}
...
fn signature_profile(methods: &[&Method]) -> BTreeSet<String> {
    methods
        .iter()
        .flat_map(|method| method.signature_types())
        .collect()
}
