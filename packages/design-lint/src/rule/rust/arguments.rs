...
use syn::{
    BinOp, Expr, ExprCall, FnArg, ImplItemFn, ItemFn, Lit, Pat, Signature, spanned::Spanned,
    visit::Visit,
};
...
impl Functions<'_> {
    fn inspect(&mut self, name: String, span: Span, signature: &Signature, block: &syn::Block) {
        ...
        finding.help =
            "derive a typed CLI parser and let the parsed command/options own validation".into();
