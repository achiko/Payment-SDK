...
mod support;
use support::{
    LockKind, boundary_call, collect_use, command_constructor, contains_await, drops,
    expression_path, fs_function, fs_options_constructor, lock_acquisition, lock_constructor,
    lock_type, references, test_attributes,
};
...
impl<'a> Blocking<'a> {
    ...
    fn method_call(&mut self, call: &ExprMethodCall) {
            ...
            && self.lock_receiver(&call.receiver) == Some(LockKind::Blocking)
        {
            ...
            && fs_options_constructor(&call.receiver, |path| self.resolve(path))
        {
            self.report(
                call.span(),
                "synchronous filesystem operation",
