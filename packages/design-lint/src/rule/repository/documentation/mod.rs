...
use crate::{
    DocumentationPolicy, Finding, LintError, Location, Result, Severity, source::Workspace,
};
...
fn walk(path: &Path, ignored: &BTreeSet<String>, output: &mut Vec<PathBuf>) -> Result<()> {
    if path.is_dir() {
        ...
        let entries =
            fs::read_dir(path).map_err(|error| LintError::io("read directory", path, error))?;
...
fn validate_example(path: &Path, findings: &mut Vec<Finding>) -> Result<()> {
    let text =
        fs::read_to_string(path).map_err(|error| LintError::io("read example", path, error))?;
