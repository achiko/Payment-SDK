...
use crate::{
    DependencyPolicy, LayerPolicy, PackageDependencyBudget, PackageLayerPolicy, Workspace,
    rule::Rule,
};
    ...
    DependencyPolicy {
        ...
        layers: vec![
            LayerPolicy {
                ...
                directory: Some("foundation".into()),
                may_depend_on: vec!["foundation".into()],
            },
            LayerPolicy {
                name: "service".into(),
                directory: Some("services".into()),
                may_depend_on: vec!["foundation".into(), "service".into()],
            },
            LayerPolicy {
                name: "product".into(),
                directory: Some("products".into()),
...
#[test]
fn exact_package_layer_overrides_directory_layer() {
    let root = fixture();
    package(&root, "services", "contract", "");
    package(
        &root,
        "services",
        "adapter",
    ...
    policy.layers.push(LayerPolicy {
        name: "contract".into(),
        directory: None,
        may_depend_on: vec!["foundation".into()],
    });
    policy.package_layers.push(PackageLayerPolicy {
        package: "adapter".into(),
        layer: "contract".into(),
    });
...
#[test]
fn rejects_package_mapping_to_unknown_layer() {
    let root = fixture();
    package(&root, "services", "adapter", "");
    let mut policy = policy();
    policy.package_layers.push(PackageLayerPolicy {
        package: "adapter".into(),
        layer: "missing".into(),
    ...
    assert!(error.to_string().contains("unknown dependency layer"));
    fs::remove_dir_all(root).unwrap();
}
...
fn package_budget_counts_distinct_local_targets_across_dependency_kinds() {
    ...
    assert!(
        values[0]
            .message
            .contains("exceeding its configured maximum of 1")
    );
    fs::remove_dir_all(root).unwrap();
}
