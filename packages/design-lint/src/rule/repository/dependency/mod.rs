...
fn layer_directory<'a>(path: &'a Path, policy: &DependencyPolicy) -> Option<&'a str> {
    path.components()
        .filter_map(|component| component.as_os_str().to_str())
        .find(|component| policy.layer(component).is_some())
}
...
impl Graph {
    fn load(paths: &[PathBuf], policy: &DependencyPolicy) -> Result<Self> {
        for mapping in &policy.package_layers {
            if policy.named_layer(&mapping.layer).is_none() {
                return Err(LintError::configuration(format!(
                    "package {:?} references unknown dependency layer {:?}",
                    mapping.package, mapping.layer
                )));
            }
        }
        ...
        for manifest in manifests {
            ...
            let package = Package {
                name: name.to_owned(),
                layer: policy.package_layer(name).map(str::to_owned).or_else(|| {
                    layer_directory(&manifest, policy)
        ...
        let roots = paths
            ...
            .map(|path| {
                normalized(path)
                    .ok_or_else(|| LintError::configuration(format!("resolve {}", path.display())))
            })
            ...
            for dependency in &package.dependencies {
                ...
                let layer_violation = if self.policy.layers.is_empty() {
                    ...
                    match (source_layer, target.layer.as_deref()) {
                        ...
                        (Some(_), None) => Some(
                            "the dependency target is not classified by repository layer policy",
                        ),
                        (None, Some(_)) => Some(
                            "the dependency source is not classified by repository layer policy",
                        ),
                        (None, None) => Some(
                            "the dependency source and target are not classified by repository layer policy",
                        ),
                        _ => None,
                    }
                };
    ...
    fn budget_finding(&self, package: &Package, rule: &'static str) -> Option<Finding> {
        ...
        review.metadata.extend([
            ...
            (
                "Observed local dependencies".into(),
                targets.len().to_string(),
            ),
        ]);
        review
            .dependencies
            .extend(targets.into_iter().map(str::to_owned));
        finding.review = Some(review);
        Some(finding)
    }
