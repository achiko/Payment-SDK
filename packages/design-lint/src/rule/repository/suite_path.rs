...
fn visit(path: &Path, rule: &'static str, findings: &mut Vec<Finding>) -> Result<()> {
        ...
        && name.contains('_')
    {
        ...
        finding.help = format!(
            "rename the suite directory to `{suggestion}` and update every manifest reference"
        );
        findings.push(finding);
    }
    let entries =
        std::fs::read_dir(path).map_err(|error| crate::LintError::io("walk", path, error))?;
