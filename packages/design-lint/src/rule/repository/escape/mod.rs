...
impl Rule for Repository {
    ...
    fn check(&self, workspace: &Workspace) -> Result<Vec<Finding>> {
        ...
        for path in workspace.paths() {
            collect(path, &mut files, &self.ignored_directories)
        ...
        for file in files {
            let text =
                fs::read_to_string(&file).map_err(|error| LintError::io("read", &file, error))?;
...
fn finding(
    ...
    value.help =
        "depend on a workspace crate that owns the files, or take the path as an explicit argument"
            .to_owned();
