...
fn filename_does_not_repeat_parent_semantic_word() {
    ...
    for path in [
        ...
        "src/memory/memory_map.rs",
        "src/net_work/socket_work.rs",
    ] {
        fs::write(root.join(path), "").unwrap();
    }
    for path in [
        "src/memory/memorial.rs",
        "src/memory/shared.rs",
    ...
    assert!(
        findings
            .iter()
            .any(|finding| finding.subject == "shared_memory")
    );
    assert!(
        findings
            .iter()
            .any(|finding| finding.subject == "memory_map")
    );
    assert!(
        findings
            .iter()
            .any(|finding| finding.subject == "socket_work")
    );
...
fn repository_test_sources_are_exempt() {
    ...
    fs::write(root.join("src/invalid-rust-source.rs"), "").unwrap();
    let workspace = Workspace::load([root.clone()]).unwrap();
    let findings = FileName.check(&workspace).unwrap();
    ...
    assert!(
        findings[0]
            .location
            .path
            .ends_with("src/invalid-rust-source.rs")
    );
    assert!(findings[0].message.starts_with("Rust source filename stem"));
    fs::remove_dir_all(root).unwrap();
}
...
",
    ...
    assert!(
        !findings
            .iter()
            .any(|finding| finding.subject == "launcher_prepare")
    );
    assert!(
        findings
            .iter()
            .any(|finding| finding.subject == "launcher_publish")
    );
    fs::remove_dir_all(root).unwrap();
}
