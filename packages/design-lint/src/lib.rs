...
pub use policy::{
    BoundaryPolicy, DependencyPolicy, DocumentationPolicy, LayerPolicy, OwnershipPolicy,
    PackageDependencyBudget, PackageLayerPolicy, Policy, SourcePolicy, SourceSelector,
    VocabularyPolicy,
};
pub use report::{Cases, Diagnostic, Markdown, Reporter};
pub use rule::{
    AccessorBloat, AsyncBlocking, BooleanState, BroadTrait, CatchAllModule, CatchAllSourcePath,
    CeremonialStructure, ConstructorOwnership, DependencyDirection, Documentation, DuplicateEntity,
    EmptyDirectory, EmptyStruct, EnvironmentAccess, FileLength, FileName, FiniteStateString,
    FolderNoun, FreeFunction, GodObject, IgnoredResult, IntegrationCandidate, ManualDispatch,
    MaximumNesting, ModelDuplication, ModulePrefix, OwnedVocabulary, PackagePrefix, PathModules,
    PlatformCommand, PrefixDirectory, ReceiverRepetition, Registry, RepositoryEscape, Rule,
    RuntimeTool, SingleFileDirectory, StructNaming, StructWordCount, SuffixRole, TestDependency,
    TestDirectory, TestName, TraitMethodCount, UnsafeBoundary,
};
    ...
    pub fn standard_with_policy(policy: Policy) -> Self {
        ...
        let mut linter = Self::new(
            Registry::new()
                ...
                .register(rule::OwnedVocabulary::new(vocabulary))
                ...
                .register(rule::EmptyStruct::new())
                .register(rule::StructWordCount::new())
    ...
    pub fn run(
        ...
        for rule in self.registry.rules() {
            let findings = rule
                .check(&workspace)?
                .into_iter()
                .filter(|finding| !suppressed(finding, rule.id()))
                .collect::<Vec<_>>();
...
fn suppressed(finding: &Finding, rule: &str) -> bool {
    let Ok(source) = std::fs::read_to_string(&finding.location.path) else {
        return false;
    };
    ...
    lines
        .get(start..=line.min(lines.len().saturating_sub(1)))
        .into_iter()
        .flatten()
        .any(|line| valid_suppression(line, rule))
}

fn valid_suppression(line: &str, rule: &str) -> bool {
    let Some(comment) = line
        .split_once("// design-lint: allow ")
        .map(|(_, value)| value)
    else {
        return false;
    };
    let Some((suppressed_rule, reason)) = comment.split_once(" -- ") else {
        return false;
    };
    suppressed_rule.trim() == rule && !reason.trim().is_empty()
}
...
"#,
        ...
        assert!(reporter.0.iter().any(
            |finding| finding.rule == "detached-constructor" && finding.subject == "construct"
        ));
        assert!(
            reporter
                .0
...
#[design_review::classify(domain = "surface")]
...
#[design_review::adapter] async fn handler(State(state): State<AppState>) { let _ = state; }
async fn unreviewed_handler(State(state): State<AppState>) { let _ = state; }
fn detached(state: AppState) { let _ = state; }
#[design_review::adapter] fn parses(value: &str) -> Result<usize, String> { Ok(value.len()) }
...
#[design_review::classify(pkg)] fn package(value: Payload) { let _ = value; }
#[design_review::classify(domain = "gpu")] fn domain(value: Payload) { let _ = value; }
#[design_review::classify(domain = "")] fn malformed(value: Payload) { let _ = value; }
"#,
            "unclassified-free-function",
        );
        assert_eq!(
        ...
        let gated = values
            .iter()
            .find(|value| value.subject == "gated")
            .unwrap();
        assert!(
            gated
                .review
                ...
                .metadata
                .iter()
                .any(|(key, value)| key == "Usage resolution"
                    && value == "unique name in scanned tree")
        );
        let package = values
            .iter()
...
struct Workspace;
struct Selected;
#[design_review::naming(reason = "external protocol vocabulary")]
struct Updated;
struct VkImageCopy2;
enum Changed { Value }
type Chosen = usize;
...
"#,
    ...
    fn reporters_injected_outputs() {
        ...
        let source = write(
            &root,
            "struct Payload;\n#[design_review::classify(pkg)] fn reviewed(value: Payload) { let _ = value; }\nfn missing(value: Payload) { reviewed(value); }",
        );
    ...
    fn empty_excluded_trees() {
        ...
        let policy = policy::SourcePolicy {
            ...
            self_packages: vec!["design-lint".into()],
            ..Default::default()
        };
    ...
    fn workspace_source_layers() {
        ...
        let workspace =
            Workspace::load_with_policy([root.join("src")], &policy).expect("load workspace");
        let paths = workspace
            .sources()
            .iter()
