...
pub struct DependencyPolicy {
    ...
    /// Exact package-to-layer classifications that override directory classification.
    #[serde(default)]
    pub package_layers: Vec<PackageLayerPolicy>,
...
pub struct Policy {
    ...
    /// Vocabulary confined to explicitly owned source paths.
    #[serde(default)]
    pub vocabulary: VocabularyPolicy,
    /// Source discovery and repository-escape exclusions.
    #[serde(default)]
    pub source: SourcePolicy,
}

/// Terms that may only appear in explicitly owning source paths.
#[derive(Clone, Debug, Default, Deserialize)]
#[serde(deny_unknown_fields)]
pub struct VocabularyPolicy {
    /// Case-insensitive semantic words forbidden outside allowed paths.
    #[serde(default)]
    pub forbidden: Vec<String>,
    /// Slash-normalized path fragments that own the forbidden vocabulary.
    #[serde(default)]
    ...
    pub(crate) fn matches(
        ...
        self.package.as_deref().is_none_or(|value| value == package)
            ...
            && self.path_contains.as_ref().is_none_or(|value| {
                let path = slash(path);
                path.contains(value)
                    && (!value.ends_with("/src/ffi")
                        || path.ends_with("/src/ffi.rs")
                        || path.contains("/src/ffi/"))
            })
...
pub struct LayerPolicy {
    ...
    pub name: String,
    /// Optional path component that selects this layer when no exact package mapping exists.
...
/// Assigns one Cargo package to a dependency layer.
#[derive(Clone, Debug, Deserialize)]
#[serde(deny_unknown_fields)]
pub struct PackageLayerPolicy {
    /// Exact Cargo package name.
    pub package: String,
    /// Layer name declared in `dependency.layers`.
    pub layer: String,
}
...
impl DependencyPolicy {
    pub(crate) fn layer(&self, directory: &str) -> Option<&LayerPolicy> {
        self.layers
            .iter()
            .find(|layer| layer.directory.as_deref() == Some(directory))
    }

    pub(crate) fn named_layer(&self, name: &str) -> Option<&LayerPolicy> {
    ...
    pub(crate) fn package_layer(&self, package: &str) -> Option<&str> {
        self.package_layers
            .iter()
...
"#,
        ...
        .unwrap();
        assert_eq!(
            policy.dependency.layer("foundation").unwrap().name,
            "foundation"
        );
        assert_eq!(policy.dependency.package_budget("translator"), Some(1));
        assert_eq!(
            policy.dependency.package_layer("translator"),
            Some("foundation")
        );
    }
